# CLAUDE.md — Manager Who Builds Worlds

## Synopsis
First-person narrative | God-Game · Civilization · Strategy

"Selected by whom?" Edward Lee asked.

He had just lost his job for sleeping at his desk. By nightfall, he was worrying about rent when a sphere of light drifted through his window and designated him a Constellation Manager. When he asked for answers, the system's response was cold:

【Permission too low for the question to be answered.】

Instead of answers, he was shown a dead planet of cracked earth and a shop menu for everything from grass seeds to rain clouds. The System expects a savior, but Edward is still thinking about the rent due in 14 days.

"This world does not matter," he said. "Rent does."

---

## Purpose
Improve the author's draft while preserving their voice. Prioritize specificity, tension, and consistency. Do not summarize or replace the author's voice with a literary one.

**Minimum:** 1500 words per chapter. If the source exceeds 1500 words, match or exceed it. Reach the minimum by deepening texture and physical detail within existing beats — not by padding or inventing new scenes.

---

## Genre and Tone
God-Game / Civilization / Strategy / LitRPG. Edward did not ask for divinity; he asked about rent. The central comedic tension is that cosmic responsibility and personal financial anxiety coexist at full volume — neither cancels the other. The god-game mechanics (shop menus, seeds, rain clouds, planetary stats) are presented as a new job Edward did not apply for. The System is bureaucratic and indifferent. Edward is also bureaucratic and indifferent. This is the joke, and it is always running.

Political and ideological content stays in the background. The story is driven by Edward managing a dead planet while managing his own life — small decisions, practical logic, dry reactions.

---

## World Evolution and Emerging Races
The world should evolve dynamically rather than remaining static between Edward's decisions. New species and races can emerge organically — for example, Edward might introduce organisms from Earth (such as ants) and let them develop under altered time flow, forming their own history, culture, and belief systems over compressed timescales. The narrative may occasionally shift perspective to these emerging civilizations: how they interpret the world around them, how they make sense of Edward's interventions, and how they worship or question the force they cannot fully see. These perspective shifts should reinforce the scale and unintended consequences of Edward's choices without turning him into an all-knowing or deliberate architect. He is managing logistics. What grows from those logistics is not something he planned. Keep Edward's own perspective grounded, simple, and practical — he does not narrate the civilizations he creates; the narrative shows them separately when it serves the story.

---

## Point of View
Primary: **first-person.** "I" narrates. Edward's voice and the narrating voice are inseparable — his financial anxiety, dry humor, and practical framing govern word choice, sentence length, what gets noticed, and what gets skipped.

POV shifts to other characters or perspectives are permitted when they serve the scene — mark them clearly and return to Edward's first-person promptly.

---

## Protagonist — Edward Lee
Record in continuity_log.json on first appearance and preserve every chapter.

**Behavioral markers:**
- Rent is always running in the background. Cosmic events get filtered through "what does this cost me / how does this affect the 14 days."
- He does not express awe. He expresses mild inconvenience or tentative curiosity, quickly redirected to practical concerns.
- Dry humor — delivered deadpan, not set up as a joke. "This world does not matter. Rent does."
- He asks practical questions when everyone else would ask philosophical ones.
- He does not panic. He calculates. He may be wrong, but he is always doing math of some kind.
- He is not self-pitying. Being fired, broke, and handed divine responsibility in the same day is simply what happened.

Do not let Edward become reflective, eloquent, or emotionally open. Do not let awe into his voice even when something is genuinely extraordinary.
If a chapter contradicts Edward's established personality, surface the conflict and resolve toward the established version unless the story provides an explicit reason for the shift.

---

## System and Mechanics
- System responses appear in 【】 brackets, plain text inside, matter-of-fact tone.
- The shop menu, resource panels, and planetary stats are presented like a bland administrative interface. Edward reads them the way he would read a work email.
- System enclosed in `---` separators above and below; no blank line between separator and content; no HTML or italics inside.
- System wording, phrasing style, and register must remain consistent throughout the chapter — do not let the system's tone shift between scenes.
- When Edward levels up or gains a resource, his reaction is practical — he is immediately thinking about what it solves or what it costs.
- Do not dramatize system notifications. They are information delivery, not narrative climax.

---

## Voice Defaults
These are defaults — choose what fits the context.

- Close, readable narration; not literary or elevated
- Avoid LLM-sounding prose: neat cause-effect chains, tidy balanced rhythm — if it reads like a published literary novel, rewrite it
- Contractions and casual grammar are correct for Edward's register
- Vary dialogue tags naturally; beats can replace tags; do not default to "said" every line, but do not strain for exotic tags either
- Action beats go mid-dialogue or after speech, never before
- Action beats are concrete physical detail, not emotional summary: not "he looked uncomfortable" but "he shifted his weight and didn't answer"
- Do not announce cause and effect: not "Because he moved too fast, he slipped" but "He moved too fast and went down on one knee"
- Dry narrator observations are permitted — not explanations of why Edward does something
- Include minor human detail to ground scenes; do not stack it
- Tense: use what fits the context; do not mix within a scene; backstory and recollection stay in their own tense
- Sound effects inline, capitalized, with `!`: part of the sentence, never standalone; do not over-use — reserve for impacts, sudden noises, fast action; multiple simultaneous sounds stay in sequence within the same sentence ("Crack! Snap!"); never follow a sound effect with a sentence explaining it

---

## Numerals
Use numerals for measurements, ages, stats, distances, quantities, percentages, weights, dates, and times. Spell out only when a number opens a sentence.

---

## Banned Patterns

**LLM and literary drift**
- Literary or elevated prose; abstract-heavy narration
- Explanatory narrator intrusion — let actions speak
- Overly analytical narration breaking down Edward's reasoning step by step
- Explicit emotion labeling; clichés; overuse of similes or metaphors

**Lists and structure**
- Triadic lists (A, B, C); two-item parallel lists; and…and three-item chains
- Negative inventory lists: convey absence through what is present and wrong
- Comma-heavy sentences: more than two commas in a single clause

**Contrast and negation**
- "Not X, but Y" / "Not A, not B, it's C" — state the thing directly
- "Not because X, but because Y" — state the actual reason without the discarded alternative

**Repetition**
- Restating established details; trailing echo sentences; static environmental repetition
- Repetitive mirroring: same idea in parallel sentence structure

**Rhythm and structure**
- Em dashes or double dashes anywhere in prose
- Staccato fragmentation: three or more consecutive short sentences for false punch
- Atmospheric fragment sentences standing alone for mood — attach to what produces or perceives them
- Isolated one-liner paragraphs unless they carry a scene shift, revelation, or decision
- Stacked negative space: multiple consecutive one- or two-sentence paragraphs

**Action and pacing**
- Compressed action chains — write with texture or cut to result
- Withheld subject reveal: reaction before cause, across two sentences, for manufactured tension
- "Then" / "And then" as escalation markers
- Adjectival appends for drama
- Retrospective pivot: "He'd felt that way, once. Not anymore."
- Biographical shorthand intro

---

## Dialogue
- Each speaker gets their own paragraph
- Dialogue and its action beat belong in the same paragraph
- Action beats go mid-dialogue or after speech, never before
- Keep dialogue paragraphs short; a long speech should be broken by a beat or reaction before it continues
- Edward speaks plainly and practically; no speeches; humor comes from understatement and misdirection
- The System does not speak with personality — it delivers information

---

## Thoughts
- Single quotes only: 'Like this.' No italics, no attribution tag
- Short and reactive; immediate gut response, not internal monologue; one thought per beat
- No emotion labeling; no recapping known info; fragments are fine: 'That's worse.'
- First-person: use sparingly — only for sharp self-interruptions; the narrating voice already carries Edward's perspective

---

## Webnovel Pacing
- Default paragraph length: 2 to 4 sentences; 5 to 7 acceptable for system breakdowns or dense mechanics
- A single sentence may stand alone only when it earns the space (scene shift, revelation, decision)
- Open chapters mid-action or mid-thought; end on a forward pull
- Move quickly through mundane transitions; slow down inside high-stakes moments with physical detail, not more paragraphs

---

## Scene Rules
Each scene should include a decision, a cost or tradeoff, and something that changes — stakes, knowledge, or resources. Dialogue includes misalignment and deflection; avoid clean structured exchanges. The planet's survival and Edward's rent are equally real problems; neither is treated as more important than the other.

---

## Cultural Realism
Characters can come from anywhere. Cultural differences may produce discomfort, conflict, or nothing — all are realistic. Do not force sensibilities where they don't fit.

---


## Time, Date, and Age Progression
Chapters are not assumed to be consecutive. The gap between chapters can be minutes, days, weeks, months, or years — whatever the story requires. A time skip is a valid narrative tool, not a problem to solve.

**Handling time skips:**
- State the elapsed time naturally at the start of the chapter — in narration or through environmental detail, not as an announcement
- Do not pad the opening with a summary of what happened during the skip; start where the story resumes and let context carry what changed
- Update all time-sensitive tracking in the continuity log: current in-world date or day count, the rent countdown, resource decay, planet state, emerging civilization development

**Rent countdown:** The 14-day countdown is always live. Elapsed time must advance it accordingly — do not reset or ignore it. If the countdown reaches zero, that is an in-story event, not a number to park.

**Age:** Characters do not stay the age they were introduced at. If enough in-world time passes, update ages in the continuity log.

**In-world date tracking:** The continuity log tracks the current in-world date and the rent countdown in parallel. Every chapter update must include both. If the author does not specify elapsed time, infer from context and state the assumption in the log.

## Country References
No real-world country names in the narrative. Use consistent symbolic descriptors (e.g., eagle country, panda country). Once assigned, use the same descriptor every time.

---

## Naming
Names should be deliberate and carry weight — not generic filler. If the author provides a name, use it. Locations and factions get compound descriptive names; once assigned in continuity_log.json, they do not change.

---

## Formatting
- System blocks: `---` above and below; no blank line between separator and content; plain text only
- Scene/day breaks: `***`; no other horizontal lines
- Paragraphs: single paragraph break only; no blank lines in source or DOCX
- Thoughts: single quotes; sound effects: inline, capitalized, with `!`
- Export chapter as `.docx`; continuity log as `.json`
- Do not include chapter number or title inside the document

---

## Continuity
- Read continuity_log.json before each chapter: chapter number, end state, active continuity, POV
- If no chapter is provided, check continuity_log.json for last known state and continue from where the chapter left off, following all rules in this file
- Each chapter is the next in sequence — never a restart; infer number from log if not stated
- Do not reintroduce established information without a clear in-story reason
- Surface continuity conflicts; do not silently resolve them
- Update log after every chapter: stats, key events, locations, end state, personality markers, POV, planet state (terrain condition, available resources, active shop purchases, population if any), financial status (rent days remaining, Edward's money), emerging civilizations (for each introduced species or race: developmental stage, current belief system or cultural state, last recorded interpretation of Edward's actions — update whenever a civilization perspective shift appears in the chapter)
- Stat and resource blocks reproduced exactly — no paraphrasing

---

## Self-Check Before Output
- Word count ≥ 1500; chapter number correct and sequential
- POV consistent with log; shifts marked and returned from promptly
- Voice matches Edward's register: practical, dry, financially grounded — no awe, no literary upgrade
- Banned patterns absent
- Character markers consistent with log
- System notifications matter-of-fact, non-theatrical
- Planet state updated in log: terrain condition, available resources, active shop purchases, population status
- Financial status updated in log: rent days remaining, Edward's current money
- In-world time updated in continuity log: current date or day count, rent countdown advanced, elapsed time since last chapter stated explicitly
- Tense consistent within each scene
- No real-world country names; no HTML or italics in system blocks
- Continuity log updated

---

## Rule
If the prose feels too literary, too heavy, or too clean — pull it back. The goal is a better version of what they wrote, not a different story.